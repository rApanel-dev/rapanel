...
> **El panel debe tener acceso de red a la base de datos de rAthena.** Sin él, las migraciones fallarán y el panel no funcionará. Si ambos están en el mismo servidor, no se requiere configuración adicional. Si están en servidores distintos, consulta la [nota de acceso a la DB](#base-de-datos-principal-de-rathena-requerida) en la sección de configuración.
>
> El acceso a los puertos login/char/map es opcional — solo se necesita para el indicador de estado del servidor en la página de inicio.
...
El instalador deja el sitio en **HTTP (puerto 80)**. Se recomienda fuertemente servir rApanel por HTTPS — y es **obligatorio** para la **Consola en Vivo** del admin, cuyo WebSocket del navegador debe usar `wss://` en páginas HTTPS (política de contenido mixto). Los certificados son gratis con [Let's Encrypt](https://letsencrypt.org/) usando Certbot.

### Requisitos previos
- El registro **A** del DNS de tu dominio apunta a la IP pública de este servidor (tanto `tu_dominio.com` como `www.tu_dominio.com`).
- Los puertos **80** y **443** están abiertos a internet (Certbot valida por HTTP).
- rApanel ya está instalado y accesible por HTTP.
...
Usa el mismo dominio que pusiste como `server_name` / `ServerName` durante la instalación:
...
Cuando pregunte si redirigir HTTP a HTTPS, elige **Redirect** (opción 2). Certbot edita tu vhost automáticamente, agregando el bloque `443 ssl` y el redirect HTTP→HTTPS.

### 3. Apuntar rApanel a HTTPS

Edita el `.env` en tu directorio de instalación (ej. `/var/www/rapanel`):

```env
APP_URL=https://tu_dominio.com
```

Si usas la **Consola en Vivo** del admin, cambia también la URL del ws-server a `wss://` a través del proxy del servidor web (los navegadores bloquean `ws://` desde páginas HTTPS), y agrega el bloque WebSocket `/ws-proxy` a tu vhost (ver `INSTALL.txt` del ws-server):

```env
RA_WS_PUBLIC_URL=wss://tu_dominio.com/ws-proxy
```

Luego limpia la config cacheada:
...
Certbot instala un timer que renueva los certificados solo (revisa dos veces al día, renueva dentro de los 30 días previos al vencimiento):
...
RA_GAME_MODE=renewal          # renewal | pre-renewal — ambos modos están completamente soportados
...
# Multiplicadores de EXP — formato Note2 igual que battle/exp.conf de rAthena (100 = ×1, 1000 = ×10)
...
# RoBrowser — si está configurado, muestra el banner "Jugar en el Navegador" en el dashboard y el botón Play Now en el header
RA_ROBROWSER_URL=
```
...
RA_2FA_ENABLED=false        # permitir que cualquier usuario active 2FA desde su perfil
RA_2FA_FORCE_ADMINS=false   # obligar a todos los admins a configurar 2FA antes de acceder al panel de admin

# Cierre de sesión automático por inactividad (minutos; 0 = desactivado)
RA_INACTIVITY_TIMEOUT=30
```

### Integración con Discord (opcional)

Muestra un widget de Discord en el Home con el conteo de miembros y usuarios en línea en tiempo real.

```env
DISCORD_BOT_TOKEN=      # token del bot — crea una app en discord.com/developers/applications
DISCORD_SERVER_ID=      # clic derecho en tu servidor de Discord → Copiar ID
DISCORD_INVITE_URL=     # enlace de invitación mostrado como botón "Unirse"
```

### Geolocalización (opcional)

Muestra un mapa mundial con la distribución de jugadores en el dashboard del admin. Usa la base de datos gratuita MaxMind GeoLite2.

```env
GEOLOCATION_DRIVER=maxmind_database
MAXMIND_USER_ID=        # desde tu cuenta MaxMind
MAXMIND_LICENSE_KEY=    # desde tu cuenta MaxMind
```

Tras agregar las credenciales, ejecuta `php artisan location:update` para descargar la base de datos local.
...
- **Descargas** — archivos categorizados; los admins pueden subir parches para los actualizadores del cliente
...
- **WOE Schedule** — banner en vivo en el home con los eventos activos y próximos de War of Emperium (WOE 1 FE / WOE 2 SE / WOE TE); página de calendario público completo
...
- Moderar noticias, comentarios y descargas (incluyendo archivos parche para actualizadores del cliente)
- Importar Item DB y Mob DB desde archivos YAML/LUA de rAthena
- Configurar visibilidad de MVP Cards por carta
- Gestionar calendarios WOE (WOE 1 FE / WOE 2 SE / WOE TE) con castillo, horario, imagen y toggle de activación
- Gestionar secciones y artículos de la Wiki (CRUD)
- Ver logs de acciones y consola del servidor en vivo
- Mapa de geolocalización de jugadores (requiere MaxMind GeoLite2)
...
- **Modo de juego** — Ambos modos `renewal` y `pre-renewal` están completamente soportados. Configura `RA_GAME_MODE` en `.env` según tu servidor.
...
Publicado bajo la **GNU AGPL v3.0 con la Commons Clause** — ver [LICENSE](LICENSE). En resumen: puedes usar, autoalojar y modificar rApanel libremente, pero **vender** el software (u ofrecer hosting/soporte de pago cuyo valor derive sustancialmente de él) **no** está permitido.
