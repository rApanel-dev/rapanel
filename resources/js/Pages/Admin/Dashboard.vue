                ...
                <div class="lg:col-span-2 bg-white dark:bg-rapanel-navy-900 rounded-xl border border-rapanel-navy-100 dark:border-white/10 shadow-sm overflow-hidden flex flex-col">
                    <div class="flex items-center justify-between px-5 py-3 border-b border-rapanel-navy-100 dark:border-white/10 shrink-0">
                        <div>
                            ...
                            <p class="text-[10px] text-rapanel-text-light/30 dark:text-white/25 mt-0.5">Based on registered country</p>
                        </div>
                        <span class="text-xs font-semibold text-rapanel-text-light/40 dark:text-white/30">
                ...
                <div class="flex flex-col gap-4">
                    ...
                    <div class="flex-1 bg-white dark:bg-rapanel-navy-900 rounded-xl border border-rapanel-navy-100 dark:border-white/10 shadow-sm overflow-hidden">
                        ...
                        <div class="px-5 py-4 space-y-3">
                            <div v-if="!topCountries?.length" class="text-sm text-rapanel-text-light/40 dark:text-white/40">
                                No country data available.
                            </div>
                            <div v-for="(country, i) in topCountries" :key="country.code" class="space-y-1">
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center gap-2 min-w-0">
                                        <span class="text-[10px] font-black text-rapanel-text-light/30 dark:text-white/25 w-4 text-right shrink-0">{{ i + 1 }}</span>
                                        <span class="text-xs font-semibold text-rapanel-text-light dark:text-white truncate">
                                            {{ countryName(country.code) }}
                                        </span>
                                        <span class="text-[10px] text-rapanel-text-light/40 dark:text-white/40 font-mono shrink-0">{{ country.code }}</span>
                                    </div>
                                    <span class="text-xs font-black text-rapanel-blue shrink-0 ml-2">{{ country.total }}</span>
                                </div>
                                <div class="h-1 bg-rapanel-navy-100 dark:bg-white/5 rounded-full overflow-hidden">
                                    <div class="h-full bg-rapanel-blue rounded-full transition-all duration-500"
                    ...
                    <div class="bg-white dark:bg-rapanel-navy-900 rounded-xl border border-rapanel-navy-100 dark:border-white/10 shadow-sm overflow-hidden">
                        ...
                        <div class="divide-y divide-rapanel-navy-100 dark:divide-white/5">
                            <div class="px-5 py-3 flex items-center justify-between">
                                <span class="text-sm text-rapanel-text-light/60 dark:text-white/50">Actions today</span>
                                <span class="text-lg font-black text-rapanel-text-light dark:text-white">{{ stats.logs_today }}</span>
                            </div>
                            <div class="px-5 py-3 flex items-center justify-between">
                                <span class="text-sm text-rapanel-text-light/60 dark:text-white/50">Total logs</span>
                                <span class="text-lg font-black text-rapanel-text-light dark:text-white">{{ stats.logs_total }}</span>
                            </div>
                            <div class="px-5 py-3">
                                <Link :href="safeRoute('admin.logs.index')"
                                    class="w-full flex items-center justify-center text-xs font-bold text-rapanel-blue hover:underline">
