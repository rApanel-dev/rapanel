...
const imagePreview = ref(props.package.image_url || null);
...
const form = useForm({
    ...
    image:         null,
    price_usd:     props.package.price_usd,
    cashpoints:    props.package.cashpoints,
    bonus_percent: props.package.bonus_percent,
    is_active:     props.package.is_active,
    is_featured:   props.package.is_featured ?? false,
    sort_order:    props.package.sort_order,
    border_color:  props.package.border_color ?? 'blue',
    _method:       'PUT',
});
...
const submit = () => {
    form.post(route('admin.donation-packages.update', props.package.id), { forceFormData: true });
};
</script>
    ...
    <AdminLayout>
        <div class="max-w-2xl space-y-6">
            ...
            <div class="flex items-center gap-4 pb-5 border-b border-rapanel-navy-100 dark:border-white/[0.055]">
                ...
                <div>
                    <h1 class="text-2xl font-display font-bold tracking-wide text-rapanel-text-light dark:text-white">{{ __('Edit Package') }}</h1>
                    <nav class="flex items-center gap-1 text-xs mt-0.5">
                        ...
                        <span class="text-rapanel-text-light/60 dark:text-white/50 truncate max-w-[200px]">{{ package.title }}</span>
                    </nav>
                </div>
            </div>
                ...
                <div class="bg-white dark:bg-rapanel-surface rounded-xl border border-rapanel-navy-100 dark:border-white/10 p-5 shadow-sm flex flex-wrap items-center gap-6">
                    ...
                    <label class="flex items-center gap-2.5 cursor-pointer select-none">
                        <button type="button" @click="form.is_active = !form.is_active"
                            :class="['relative w-10 h-5 rounded-full transition-colors duration-200', form.is_active ? 'bg-rapanel-blue' : 'bg-rapanel-navy-100 dark:bg-white/20']">
                    ...
                    <label class="flex items-center gap-2.5 cursor-pointer select-none">
                        <button type="button" @click="form.is_featured = !form.is_featured"
                            :class="['relative w-10 h-5 rounded-full transition-colors duration-200', form.is_featured ? 'bg-rapanel-gold' : 'bg-rapanel-navy-100 dark:bg-white/20']">
