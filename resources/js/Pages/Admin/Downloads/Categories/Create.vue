...
const submit = () => form.post(route('admin.download-categories.store'));
</script>
...
<template>
    <AdminLayout>
        <div class="flex items-center gap-3 mb-6">
            ...
            <h1 class="text-xl font-bold text-rapanel-navy-900 dark:text-white">{{ __('New Category') }}</h1>
        </div>
        ...
        <form @submit.prevent="submit" class="space-y-6 max-w-lg">
            <div class="bg-white dark:bg-rapanel-navy-900 rounded-xl border border-rapanel-navy-100 dark:border-white/10 shadow-sm p-6 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-semibold text-rapanel-navy-900 dark:text-white mb-1.5">
                            {{ __('Icon') }} <span class="font-normal text-rapanel-text-light dark:text-rapanel-text-dark">(emoji)</span>
                        </label>
